...
import '../../../Controllers/wallet/sending_coins/all_coins_controller/arbcoin_controller.dart';
...
import '../send_coins_screens/send_arb.dart';

class ArbCoinScreen extends StatelessWidget {
  const ArbCoinScreen({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    ...
    return GetBuilder<ArbCoinController>(
      init: ArbCoinController(),
      builder: (_) {
        return MediaQuery(
          data: mqDataNew,
          child: Scaffold
  ...
  Widget buttonPart(BuildContext context, ArbCoinController _) {
    return   Container(
      width: Get.width,
      height: Get.height / 8,
      // color: Colors.red,
  ...
  ////////////////////////////////////////////////////////////////////////
  Widget coin(BuildContext context, ArbCoinController _) {
    final myTextValue = _.balanceInBNB ?? 0.0; // Ensure myText is parsed as a double
    ...
    return Column(
      children: [
        ...
        MyText(
          "$formattedString ARB", // Display formattedString with 6 decimal places or "0.00"
          fontWeight: FontWeight.w600,
          fontSize: 20,
        )
      ],
  ...
  Widget lastContainer(BuildContext context, ArbCoinController _) {
    return Expanded(
      child: Container(
        decoration: BoxDecoration(
  ...
  Widget historyTab(BuildContext context, ArbCoinController _) {
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (c, i) {
                ...
                Color amountColor =
                amount.startsWith('-') ? Colors.red : Colors.green; // Set color based on amount sign
                String assetImage =
                amount.startsWith('-') ? AppAssets.historyCoinLogo : AppAssets.reciveCoinHistory; // Set image based on amount sign
